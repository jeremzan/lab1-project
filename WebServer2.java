    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            ...
            BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(clientSocket.getOutputStream()))) {
                ...
                // Rewrite the path to remove "/.."
                String[] pathSegments = resourcePath.split("/");
                StringBuilder rewrittenPath = new StringBuilder();
                for (int i = 0; i < pathSegments.length; i++) {
                    if (pathSegments[i].equals("..") && rewrittenPath.length() > 0) {
                        // Remove the last segment if ".." is found
                        int lastSegmentIndex = rewrittenPath.lastIndexOf("/");
                        if (lastSegmentIndex != -1) {
                            rewrittenPath.delete(lastSegmentIndex, rewrittenPath.length());
                        }
                    } else if (!pathSegments[i].isEmpty() && !pathSegments[i].equals(".")) {
                        // Append valid segments to the rewritten path
                        if (rewrittenPath.length() > 0) {
                            rewrittenPath.append("/");
                        }
                        rewrittenPath.append(pathSegments[i]);
                    }
                }
